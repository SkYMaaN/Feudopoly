...
export class Board extends Phaser.Scene {
    ...
    applyState(state) {
        ...
        const incomingIds = new Set(state.players.map(player => player.playerId));

        this.players
            .filter(player => !incomingIds.has(player.playerId))
            .forEach(player => {
                player.sprite.destroy();
            });

        this.players = this.players.filter(player => incomingIds.has(player.playerId));

        state.players.forEach((playerState, index) => {
            let player = this.players.find(item => item.playerId === playerState.playerId);

            if (!player) {
                player = this.createPlayer(playerState.playerId, playerState.displayName, playerState.position);
        ...
        this.activeTurnIndex = state.activeTurnIndex ?? 0;
    ...
    refreshTurnUI() {
        ...
        const current = this.players[this.activeTurnIndex % this.players.length];
    ...
    playDiceResult(payload) {
        const player = this.players.find(item => item.playerId === payload.playerId);
        if (!player) {
            return;
        }
